...
provider <-  sf::st_read(
  ...
  query= "
    SELECT *
  FROM dc_health_behavior_diet.va_pl_sgrph_2021_dentist_offices"
)
...
provider <- provider %>% distinct(location_name, location, .keep_all = TRUE)
...
# save new data
```{r}
write.csv(provider[, c("ID", "location_name", "lat", "lon", "doctors")], "provider.csv", row.names = FALSE)
